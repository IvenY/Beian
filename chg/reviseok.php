...
$upztstmt = $pdo ->prepare('UPDATE `rs_opt_state` SET czzt= ? WHERE usrId = ? AND czlx = ?');
$upztstmt -> execute(array("101",$userid,$mkdm));
if($ifresult){
...
if($result && $result2){
	echo "<script>alert('保存成功，返回桌面')</script>";
	echo "<script>window.location.href='chgZt.php'</script>";
}else{
	echo '抱歉！添加数据失败：'.'<br />';
