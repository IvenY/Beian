...
if($_SERVER['REQUEST_METHOD']=='POST'){
	...
	if(move_uploaded_file($tmp_name,$path1.$showphpath)){
		...
		$row3 = $stmt3 -> fetch();
		if(!$row3){
			$stmt1 = $pdo -> prepare('INSERT INTO `rs_usr_ver_bill` (mkdm,usrId,scsj,bclj) VALUES (?,?,?,?)');
			$arr = array($mkdm,$userid,$scsj,$showphpath);
			$stmt1 -> execute($arr);
		}else{
			$stmt1 = $pdo -> prepare('UPDATE `rs_usr_ver_bill` SET bclj=? WHERE usrId=? && mkdm = ?');
...
<body>
	<div id="upldiv" class="dxui_dialog_alert">
			...
			<form id="upload_form" enctype="multipart/form-data" method="post"
				name="form1"  action="">
				<div class="dxui_dialog_bd" style="padding: 1em 1.5em 0">
					<input type="file" name="photo" id="upload" />
				</div>
				<input hidden="true" name="mkdm" value="<?php echo $mkdm?>"/>
				<div class="dxui_dialog_ft" style="padding-top: 2em">
